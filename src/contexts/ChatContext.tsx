...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [explanationLevel, setExplanationLevel] =
    useState<ExplanationLevel>('highlevel')
  ...
  const testApiConnection = async (key: string, model: string) => {
    try {
      // Don't check availableModels here as they might not be loaded yet
      // The testConnection function will handle model validation
      if (!model) {
        return false
      }
